...
class PurchaseController extends Controller
{
    ...
    {
        $purchases = Purchase::paginate(10); // Usar paginación
...
public function store(Request $request)
{
    ...
    try {
        ...
        DB::commit();
        return redirect()->route(auth()->user()->role . '.purchases.show', $purchase)->with('success', 'Compra registrada exitosamente');
        ...
        try {
            ...
            foreach ($purchase->items as $item) {
                $inventory = Inventory::firstOrCreate(
                    ['product_id' => $item->product_id],
                    ['quantity' => 0, 'minimum_stock' => 1, 'unit' => 'unidad']
                );
            ...
            DB::commit();
        return redirect()->route(auth()->user()->role . '.purchases.index')
            ->with('success', 'Compra completada exitosamente.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, Purchase $purchase)
{
    ...
    return redirect()->route('chef.purchases.index')->with('success', 'Compra actualizada exitosamente');
}
}
