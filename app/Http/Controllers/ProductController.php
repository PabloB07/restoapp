...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'category' => 'required|in:comida,bebestible,postre,ingrediente,verdura,fruta,jugo,barra', // Asegúrate de que 'jugo' esté incluido
            'available' => 'boolean',
        ...
        try {
            ...
            Inventory::create([
                ...
                'status' => $validatedData['inventory']['quantity'] > $validatedData['inventory']['minimum_stock'] ? 'ok' : 'alert'
            ]);

            DB::commit();

            return redirect()->route(auth()->user()->role . '.products.index')->with('success', 'Producto creado exitosamente.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, Product $product)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route(auth()->user()->role . '.products.index')->with('success', 'Producto actualizado exitosamente');
    ...
    public function destroy(Product $product)
    {
        try {
            $product->delete();
            return redirect()->route(auth()->user()->role . '.products.index')->with('success', 'Producto eliminado exitosamente');
        } catch (\Exception $e) {
