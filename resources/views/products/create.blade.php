...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <form action="{{ route(auth()->user()->role . '.products.store') }}" method="POST">
                @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                ...
                                <option value="jugo" {{ old('category') == 'jugo' ? 'selected' : '' }}>Jugo</option> <!-- Asegúrate de que 'jugo' esté incluido -->
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="inventory[unit]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                ...
                                <option value="caja" {{ old('inventory.unit') == 'caja' ? 'selected' : '' }}>Caja</option>
                                <option value="lt" {{ old('inventory.unit') == 'lt' ? 'selected' : '' }}>Litros</option>
                                <option value="gr" {{ old('inventory.unit') == 'gr' ? 'selected' : '' }}>Gramos</option>
                            </select>
                        </div>
                    </div>
                </div>
