...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Editar Item de Inventario</h2>
        ...
        <div class="bg-white overflow-hidden shadow-lg rounded-lg">
            <div class="p-6">
                <form action="{{ route(auth()->user()->role . '.inventory.update', $inventory) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="unit"
                                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200"
                                    required>
                                <option value="unidad" {{ $inventory->unit == 'unidad' ? 'selected' : '' }}>Unidad</option>
                                <option value="kg" {{ $inventory->unit == 'kg' ? 'selected' : '' }}>Kilogramos</option>
                                <option value="caja" {{ $inventory->unit == 'caja' ? 'selected' : '' }}>Caja</option>
                                <option value="lt" {{ $inventory->unit == 'lt' ? 'selected' : '' }}>Litros</option>
                                <option value="gr" {{ $inventory->unit == 'gr' ? 'selected' : '' }}>Gramos</option>
                            </select>
                            @error('unit')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
